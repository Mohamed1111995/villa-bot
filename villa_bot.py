...
async def receive_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data["photo_count"] >= 44:
        await update.message.reply_text("⚠️ الحد الأقصى 44 صورة. اكتب /done للمتابعة.")
        return PHOTO
    ...
    await update.message.reply_text(f"✅ صورة {data['photo_count']} تم حفظها\nارسل أخرى أو اكتب /done")
    return PHOTO

async def done_photos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if user_data_temp[user_id]["photo_count"] == 0:
        await update.message.reply_text("❌ لم تُرسل أي صور! ارسل صورة أولاً.")
        return PHOTO
    count = user_data_temp[user_id]["photo_count"]
    await update.message.reply_text(f"📝 تم استقبال *{count}* صورة\n\nاكتب الملاحظات الآن:", parse_mode="Markdown")
    return NOTES
...
async def receive_notes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i in range(1, data["photo_count"] + 1):
        for path in [data.get(f"photo_{i}"), data.get(f"photo_{i}", "").replace("_processed.jpg", ".jpg")]:
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            PHOTO: [
                ...
                MessageHandler(filters.Regex("^/done$"), done_photos),
            ],
